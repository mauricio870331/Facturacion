...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 07-07-2017 a las 02:48:12
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `actualizarActividadEconomica` (`id_actividad` INT, `nombre` VARCHAR(20), `descripcion` VARCHAR(100))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `anularFactura` (IN `idfactura` VARCHAR(20), IN `concepto` VARCHAR(200), IN `id_usuario` VARCHAR(15), IN `fecha` DATETIME, IN `transacion` VARCHAR(30))  NO SQL
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `cargarImpuestos` ()  NO SQL
BEGIN
SELECT * FROM impuestos;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `cargarModulos` (IN `vendedor` VARCHAR(20))  NO SQL
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `eliminarActividadEconomica` (IN `id_actividad` INT)  BEGIN
IF EXISTS ( SELECT ae.id_actividad_economica
FROM actividades_economicas AS ae
WHERE ae.id_actividad_economica = id_actividad) THEN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `existUser` (IN `user` VARCHAR(20), IN `pass` VARCHAR(100))  NO SQL
BEGIN
select * from usuarios_app where id_usuario = user and password = pass
and id_estado_usuario = 1;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `insertarActividadEconomica` (`nombre` VARCHAR(20), `descripcion` VARCHAR(100))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertarConsecutivoImprimible` (IN `id_consecutivo` INT(11), IN `prefijo` VARCHAR(10), IN `consecutivo` INT(11), IN `id_tipo_imprimible` INT(11), IN `id_empresa` VARCHAR(15), IN `transacion` VARCHAR(30))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertarDetalleFacturaCotizacion` (IN `id_producto_servicio` INT(11), IN `cantidad` INT(11), IN `id_factura` VARCHAR(20), IN `id_impuesto` INT(2), IN `transacion` VARCHAR(30), IN `valor` FLOAT, IN `descuento` FLOAT)  NO SQL
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertarFactura` (IN `id_factura` VARCHAR(20), IN `fecha` DATETIME, IN `id_resolucion` VARCHAR(15), IN `id_cliente` INT(11), IN `subtotal` FLOAT, IN `iva` FLOAT, IN `total` FLOAT, IN `id_estado_factura` INT(11), IN `id_empresa` VARCHAR(15), IN `id_usuario` VARCHAR(15), IN `id_termino_pago` INT(11), IN `vencimiento` DATE, IN `nota` VARCHAR(100), IN `transacion` VARCHAR(30))  NO SQL
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertarFacturaAnulada` (IN `id_factura` VARCHAR(20), IN `concepto` VARCHAR(200), IN `id_usuario` VARCHAR(15), IN `fecha` DATE, IN `transacion` VARCHAR(30))  NO SQL
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertarReciboCaja` (IN `factura` VARCHAR(20), IN `consec` VARCHAR(11), IN `fech` DATE, IN `empresa` VARCHAR(15), IN `cliente` INT, IN `val` FLOAT, IN `concept` VARCHAR(200), IN `chek` VARCHAR(50), IN `tipo_pago` INT, IN `trans` VARCHAR(30), IN `id_hospedaje` INT(11))  NO SQL
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertarResolucionFactura` (IN `id_resolucion` VARCHAR(15), IN `fecha` DATE, IN `prefijo` VARCHAR(15), IN `consecutivo_inicial` INT(11), IN `consecutivo_final` INT(11), IN `id_tipo_resolucion` INT(11), IN `activo` BOOLEAN, IN `id_empresa` VARCHAR(15), IN `transacion` VARCHAR(30))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertarRetencionPago` (IN `id_factura` VARCHAR(15), IN `id_retencion` INT(11), IN `transacion` VARCHAR(30), IN `valor` FLOAT)  NO SQL
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertarUsuarioApp` (IN `id_usuario` VARCHAR(15), IN `id_tipo_identificacion` INT(11), IN `id_empresa` VARCHAR(15), IN `id_rol` INT(11), IN `password` VARCHAR(100), IN `nombres` VARCHAR(50), IN `apellidos` VARCHAR(50), IN `id_estado_usuario` INT(11), IN `transacion` VARCHAR(30))  NO SQL
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `listarActividadEconomica` ()  BEGIN
SELECT * FROM actividades_economicas;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listarCiudad` ()  NO SQL
BEGIN
SELECT * FROM ciudades;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listarCotizacion` (IN `empresa` VARCHAR(15))  BEGIN
SELECT * FROM facturas where id_empresa = empresa and id_estado_factura = 6 order by fecha DESC;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listarEntidadesBancarias` ()  NO SQL
SELECT * from entidades_bancarias$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listarFactura` (IN `empresa` VARCHAR(15))  BEGIN
SELECT * FROM facturas where id_empresa = empresa and id_estado_factura <> 6 order by fecha DESC;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listarFranquicia` ()  NO SQL
SELECT * from franquicias$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listarMotivo` ()  NO SQL
select * from motivos_estadia$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listarNacionalidad` ()  NO SQL
select * from nacionalidades$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listarProductoServicio` (IN `empresa` VARCHAR(15))  NO SQL
BEGIN
SELECT * FROM productos_servicios where id_empresa = empresa;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listarResolucionFactura` ()  BEGIN
SELECT * FROM resoluciones_facturas;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listarRetencion` (IN `empresa` VARCHAR(15))  NO SQL
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `listarRol` ()  NO SQL
SELECT * FROM `roles` WHERE nombre <> "Administrador"$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listarTerminoPago` ()  NO SQL
BEGIN 
 SELECT * FROM terminos_de_pago;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listarTipoDeHabitacion` (IN `empresa` VARCHAR(15))  NO SQL
SELECT * from tipos_de_habitacion where id_empresa = empresa$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listarTipoDeIdentificacion` ()  NO SQL
    DETERMINISTIC
BEGIN
SELECT * FROM tipos_de_identificacion;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listarTipoDepago` ()  NO SQL
BEGIN 
 SELECT * FROM tipos_de_pagos;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listarTipoImprimible` ()  NO SQL
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `listarTipoResolucion` ()  NO SQL
SELECT * FROM tipos_resoluciones$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listarUnidadMedida` ()  NO SQL
select * from unidades_medida$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listarUsuarioApp` ()  NO SQL
select * from usuarios_app$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listDetalleFactura` (IN `idFactura` VARCHAR(20))  NO SQL
...
(1, 'FVH', 175, 1, '51744384-1', '51744384-1_1'),
(2, '', 1, 2, '51744384-1', '51744384-1_2'),
(3, '', 159, 3, '51744384-1', '51744384-1_3');
...
('a', 1, '51744384-1', 3, '[a]', 'a', 'a', 1, '51744384-1_6'),
('e', 2, '51744384-1', 3, '[e]', 'e', 'e', 1, '51744384-1_8'),
('m', 1, '51744384-1', 3, '[m]', 'm', 'm', 1, '51744384-1_5'),
('q', 1, '51744384-1', 3, '[q]', 'q', 'q', 1, '51744384-1_10'),
('r', 2, '51744384-1', 3, '[r]', 'r', 'r', 1, '51744384-1_9'),
('t', 2, '51744384-1', 3, '[t]', 't', 't', 1, '51744384-1_10'),
('w', 2, '51744384-1', 3, '[w]', 'w', 'w', 1, '51744384-1_7');
...
--
ALTER TABLE `detalle_factura_cotizacion`
  MODIFY `id_detalle` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=212;
--
...
--
ALTER TABLE `facturas_anuladas`
  MODIFY `id_factura_anulada` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;
--
...
--
ALTER TABLE `pagos`
  MODIFY `id_pago` int(15) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=159;
--
...
--
ALTER TABLE `recibos_caja`
  MODIFY `id_recibo_caja` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=159;
--
